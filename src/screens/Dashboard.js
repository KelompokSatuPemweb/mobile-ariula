...
export default function Dashboard({ keLogin, navigasi }) {
    return (
        <View style={styles.container}>
            <View style={styles.header}>
                <Text style={styles.welcome}>Halo, Siswa!</Text>
                <TouchableOpacity onPress={keLogin}>
                    <Text style={styles.logout}>Logout</Text>
                </TouchableOpacity>
            </View>

            <ScrollView contentContainerStyle={styles.menuContainer}>
                {/* Tambahkan onPress untuk setiap MenuCard */}
                <MenuCard 
                    title="Tabungan" 
                    color="#2ecc71" 
                    icon="💰" 
                    onPress={() => navigasi('tabungan')} 
                />
                <MenuCard 
                    title="Nilai Siswa" 
                    color="#e67e22" 
                    icon="📝" 
                    onPress={() => navigasi('nilai')} 
                />
                <MenuCard 
                    title="Jadwal" 
                    color="#9b59b6" 
                    icon="📅" 
                    onPress={() => navigasi('jadwal')} 
                />
                <MenuCard 
                    title="Profil" 
                    color="#34495e" 
                    icon="👤" 
                    onPress={() => Alert.alert("Segera Hadir", "Fitur Profil masih dikembangkan.")} 
                />
            </ScrollView>
        </View>
    );
